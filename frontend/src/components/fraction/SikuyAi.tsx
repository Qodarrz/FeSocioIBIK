...
const LoaderAi = (
  <div className="bubble bubble-ai rounded-md">
    <span className="sender sender-ai">AgenPeduli</span>
    ...
    >
      <div className="w-full py-2 border-bottom items-center flex justify-between">
        <div className="flex gap-2 items-center">
          <span className="text-primary dark:text-textDark font-bold">AgenPeduli</span>
        </div>
        <button 
            ...
            return (
              !chat.hideInChat && (
                ...
                >
                  ...
                  >
                    {chat.role === "model" ? "AgenPeduli" : "User"}
                  </span>
      ...
      <form className="w-full gap-2 flex items-center" onSubmit={handleSubmit}>
        <div className="w-full">
          <input
            type="text"
            ref={inputRef}
            placeholder="Tanya Dengan AgenPeduli.."
            required
...
const ButtonAi: React.FC<ButtonAiProps> = ({ aiActive, setAiActive }) => {
  ...
  return (
    ...
    >
      <div 
        className='bg-[#016B61] span-hover-greenai text-white' 
        style={{ opacity: firstSpan ? "1" : "0" }}
      >
        <span>AgenPeduli</span>
      </div>
      <div 
        className='rounded-md btn-ai flex items-center justify-center text-white shadow-sm bg-[#016B61] dark:bg-[#016B61]' 
        onClick={() => setAiActive(!aiActive)}
      >
        <BotMessageSquare />
      </div>
