...
function CarouselItem({ item, index, itemWidth, round, trackItemOffset, x, transition }: CarouselItemProps) {
  ...
  return (
    <motion.div
      ...
      style={{
        width: itemWidth,
        background: item.image ? `linear-gradient(to bottom, rgba(0,0,0, .7), rgba(0,0,0,0)), url(${item.image}) center/cover no-repeat` : undefined,
        height: round ? itemWidth : '200px',
        rotateY: rotateY,
    ...
    >
      ...
      <div className="p-5">
        {
          item.image ? null : (
            <><div className="mb-1 font-black text-lg text-white">{item.title}</div><p className="text-sm text-white">{item.description}</p></>
          )
        }
      </div>
    </motion.div>
